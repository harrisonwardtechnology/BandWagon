...
export async function refreshRouteAssistRecommendations(organizationId:string,driverPersonId:string){
 ...
 for(const ride of rides.rows){
  ...
  for(const req of open.rows){
   ...
   if(row.status==='recommended'||row.status==='viewed')out.push({...row,event_title:req.title,passenger_name:req.passenger_name,routing_provider:route.provider,routing_cache_hit:Boolean(route.cacheHit)});
  }
 }
