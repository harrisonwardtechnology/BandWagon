...
export async function refreshRouteAssistRecommendations(organizationId:string,driverPersonId:string){
 ...
 const open=await db.query(`select rr.id,rr.event_id,rr.direction,rr.seats_needed,rr.requested_pickup_at,e.starts_at,e.title,pl.generalized_latitude as lat,pl.generalized_longitude as lng from ride_requests rr left join events e on e.id=rr.event_id left join private_locations pl on pl.id=rr.pickup_location_id where rr.organization_id=$1 and rr.status='open' and rr.passenger_person_id<>$2 and rr.requester_person_id<>$2`,[organizationId,driverPersonId]);
 const out:any[]=[];
 for(const ride of rides.rows){
  ...
  for(const req of open.rows){
   ...
   const extraMinutes=Math.ceil(route.durationSeconds/60+2);
   ...
   if(row.status==='recommended'||row.status==='viewed')out.push({...row,event_title:req.title,seats_needed:req.seats_needed,direction:req.direction,routing_provider:route.provider,routing_cache_hit:Boolean(route.cacheHit)});
  }
 }
