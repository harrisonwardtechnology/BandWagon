...
export async function listAdminOrganizations(identity:SessionIdentity){
  const db=dbRequired();
  return (await db.query(`select o.id,coalesce(o.display_name,o.name) as name,o.slug,m.role from memberships m join organizations o on o.id=m.organization_id where m.person_id=$1 and m.group_id is null and m.status='active' and m.role in ('owner','admin','manager') and o.status='active' order by name`,[identity.personId])).rows;
}
...
export async function getOperationsDashboard(identity:SessionIdentity,organizationId:string){
  ...
  const [organization,rideStats,driverStats,pending,expiring,safety,aiUsage,routingUsage,routeAssistStats,support,validatedDrivers,requirements]=await Promise.all([
    ...
    db.query(`select provider,sum(request_count)::int as requests,sum(cache_hits)::int as cache_hits,sum(fallback_count)::int as fallbacks,sum(estimated_cost_microusd)::bigint as cost_microusd from routing_usage_daily where organization_id=$1 and usage_date>=date_trunc('month',current_date)::date group by provider order by provider`,[organizationId]),
    db.query(`select count(*) filter (where created_at>=date_trunc('month',now()))::int as recommendations,count(*) filter (where created_at>=date_trunc('month',now()) and status='accepted')::int as offers,count(*) filter (where created_at>=date_trunc('month',now()) and status='dismissed')::int as dismissed from driver_ride_recommendations where organization_id=$1`,[organizationId]),
  ...
  const org=organization.rows[0],rides=rideStats.rows[0]||{},drivers=driverStats.rows[0]||{},safetyStats=safety.rows[0]||{},routeAssist=routeAssistStats.rows[0]||{};
  ...
  const aiCostMicrousd=aiRows.reduce((sum:number,r:any)=>sum+r.costMicrousd,0),aiCostCents=Math.round(aiCostMicrousd/10000);
  const routingRows=routingUsage.rows.map((r:any)=>({...r,requests:Number(r.requests||0),cacheHits:Number(r.cache_hits||0),fallbacks:Number(r.fallbacks||0),costMicrousd:Number(r.cost_microusd||0),costCents:Math.round(Number(r.cost_microusd||0)/10000)}));
  const routingCostMicrousd=routingRows.reduce((sum:number,r:any)=>sum+r.costMicrousd,0),routingCostCents=Math.round(routingCostMicrousd/10000);
  ...
  const estimatedOperatingCostCents=rideCostCents+driverValidationCostCents+aiCostCents+routingCostCents,supportCents=Number(support.rows[0]?.support_cents||0);
  return {
    organization:org,
    rides:{today:Number(rides.today_rides||0),month:Number(rides.month_rides||0)},
    ...
    routing:{rows:routingRows,totalCostMicrousd:routingCostMicrousd,totalCostCents:routingCostCents,totalRequests:routingRows.reduce((s:number,r:any)=>s+r.requests,0),cacheHits:routingRows.reduce((s:number,r:any)=>s+r.cacheHits,0),fallbacks:routingRows.reduce((s:number,r:any)=>s+r.fallbacks,0),recommendations:Number(routeAssist.recommendations||0),offers:Number(routeAssist.offers||0),dismissed:Number(routeAssist.dismissed||0)},
    economics:{rideCostCents,driverValidationCostCents,aiCostCents,routingCostCents,estimatedOperatingCostCents,supportCents,coveragePercent:estimatedOperatingCostCents?Math.round(supportCents/estimatedOperatingCostCents*100):0,validatedDrivers:Number(validatedDrivers.rows[0]?.count||0)},
...
export async function reviewDriverRequirement(identity:SessionIdentity,input:{organizationId:string;driverPersonId:string;requirementType:string;approve:boolean;expiresAt?:string|null;notes?:string|null}){
  ...
  const result=await db.query(`insert into driver_requirement_status (organization_id,driver_person_id,requirement_type,status,reviewed_by_person_id,reviewed_at,expires_at,notes,updated_at) values ($1,$2,$3,$4,$5,now(),$6,$7,now()) on conflict (organization_id,driver_person_id,requirement_type) do update set status=excluded.status,reviewed_by_person_id=excluded.reviewed_by_person_id,reviewed_at=now(),expires_at=coalesce(excluded.expires_at,driver_requirement_status.expires_at),notes=excluded.notes,updated_at=now() returning *`,[input.organizationId,input.driverPersonId,input.requirementType,approvedStatus,identity.personId,input.expiresAt||null,input.notes||null]);
...
export async function updateDriverRequirements(identity:SessionIdentity,input:any){
  await assertAdmin(identity,input.organizationId);const db=dbRequired();
  const result=await db.query(`update organization_driver_requirements set minimum_driver_age=$1,district_volunteer_mode=$2,driver_license_mode=$3,insurance_mode=$4,manual_approval_required=$5,suspend_on_expired_credentials=$6,ai_document_review_enabled=$7,updated_at=now() where organization_id=$8 returning *`,[Math.max(18,Math.min(99,Number(input.minimumDriverAge||18))),input.districtVolunteerMode||"not_used",input.driverLicenseMode||"optional",input.insuranceMode||"optional",Boolean(input.manualApprovalRequired),input.suspendOnExpiredCredentials!==false,input.aiDocumentReviewEnabled!==false,input.organizationId]);
  return result.rows[0];
}
