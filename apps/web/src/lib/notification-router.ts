...
export type NotificationType=
  ...
  |"last_minute_cancellation"|"pickup_changed"|"safety_alert"|"credential_expiring"|"otp"|"platform_test"|string;
...
type Policy={urgency:NotificationUrgency;push:boolean;emailFallback:boolean;smsFallback:boolean;smsImmediate:boolean;smsOnly?:boolean;};
...
async function recipientContext(request:NotificationRequest){
  ...
  const preferences={push_enabled:true,email_enabled:true,sms_enabled:true,sms_for_critical_only:true,reminder_push_enabled:true,reminder_email_enabled:false,reminder_sms_enabled:false};
  if(request.personId){const pref=await db.query(`select push_enabled,email_enabled,sms_enabled,sms_for_critical_only,reminder_push_enabled,reminder_email_enabled,reminder_sms_enabled from notification_preferences where person_id=$1 and organization_id is not distinct from $2 limit 1`,[request.personId,request.organizationId||null]);if(pref.rows[0])Object.assign(preferences,pref.rows[0]);}
  let email=request.email||null;if(!email&&request.personId){const result=await db.query(`select normalized_email from emails where person_id=$1 and verified_at is not null order by verified_at desc limit 1`,[request.personId]);email=result.rows[0]?.normalized_email||null;}
...
function shouldUseReminderPreference(type:string){return type==="reminder_24h"||type==="reminder_1h";}

export async function routeNotification(request:NotificationRequest){
  const policy=policyFor(request.notificationType,request.forceUrgency);const correlationId=request.correlationId||crypto.randomUUID();const context=await recipientContext(request);const prefs=context.preferences;const reminder=shouldUseReminderPreference(request.notificationType);
  const pushAllowed=policy.push&&prefs.push_enabled&&(!reminder||prefs.reminder_push_enabled);const emailAllowed=prefs.email_enabled&&(!reminder||prefs.reminder_email_enabled);
  const smsAllowed=prefs.sms_enabled&&(!prefs.sms_for_critical_only||policy.urgency==="critical")&&(!reminder||prefs.reminder_sms_enabled||policy.urgency==="critical");
  const result:any={correlationId,notificationType:request.notificationType,urgency:policy.urgency,plan:{pushAllowed,emailAllowed,smsAllowed,smsImmediate:policy.smsImmediate,smsFallback:policy.smsFallback},push:{attempted:0,accepted:0,failed:0},email:{attempted:false,accepted:false,skipped:false},messaging:{attempted:false,accepted:false}};
  ...
  const shouldEmail=policy.emailFallback&&emailAllowed&&Boolean(context.email)&&(!pushAvailable||policy.urgency==="critical");
...
export function notificationPolicySummary(){return Object.entries(POLICIES).map(([type,policy])=>({type,...policy}));}
