...
export async function getProductDashboard(identity: SessionIdentity) {
  const db = dbRequired();
  const people = await managedPeople(identity);
  ...
  const requests = managedIds.length ? await db.query(
    `select rr.*,e.title as event_title,e.starts_at as event_starts_at,p.display_name as passenger_name,
     ...
     where rr.passenger_person_id=any($1::uuid[])
     order by coalesce(e.starts_at,rr.requested_pickup_at,rr.created_at) desc limit 100`,
    [managedIds]
  ) : { rows: [] };
  const rides = managedIds.length ? await db.query(
    `select r.*,e.title as event_title,e.starts_at as event_starts_at,d.display_name as driver_name,
            ...
            coalesce(
              (select rr2.pickup_location_id
               ...
               order by case when rr2.passenger_person_id=$1 then 0 else 1 end,rr2.created_at
               limit 1),
  ...
  const driverProfiles = await db.query(`select * from driver_profiles where person_id=$1`, [identity.personId]);
  return {
    identity,
    people,
...
export async function createUserRideRequest(identity: SessionIdentity, input: {
  ...
  const ride = await createRideRequest({
    ...
    passengerPersonId:input.passengerPersonId,
    direction:input.direction,
    seatsNeeded:input.seatsNeeded || 1,
  ...
  if (pickupGeo) {
    const location = await createPrivateLocation({
      ...
      generalizedArea:pickupGeo.generalizedArea,
      revealPolicy:'matched_driver',
    });
  ...
  if (dropoffGeo) {
    const location = await createPrivateLocation({
      ...
      latitude:dropoffGeo.latitude,
      longitude:dropoffGeo.longitude,
      generalizedArea:dropoffGeo.generalizedArea,
      revealPolicy:'matched_driver',
    });
    dropoffLocationId = location.id;
  }
  if (pickupLocationId || dropoffLocationId) {
    await attachLocationsToRideRequest({
...
export async function offerRideAsCurrentUser(identity: SessionIdentity, input: { rideRequestId:string; seatsOffered?:number; note?:string|null; proposedPickupAt?:string|null }) {
  const db = dbRequired();
  const profile = await db.query(`select 1 from driver_profiles where person_id=$1 and status='active' limit 1`, [identity.personId]);
  if (!profile.rowCount) throw new Error("Enable your driver profile before offering a ride");
  return createRideOffer({
    rideRequestId:input.rideRequestId,
    driverPersonId:identity.personId,
    seatsOffered:input.seatsOffered || 1,
    note:input.note || null,
    proposedPickupAt:input.proposedPickupAt || null,
  });
}

export async function acceptOfferAsCurrentUser(identity: SessionIdentity, input: { rideRequestId:string; offerId:string }) {
  const db = dbRequired();
  const request = await db.query(`select requester_person_id,passenger_person_id from ride_requests where id=$1`, [input.rideRequestId]);
  if (!request.rowCount) throw new Error("Ride request not found");
  const row = request.rows[0];
  if (row.requester_person_id !== identity.personId) {
    await assertManagedPassenger(identity,row.passenger_person_id);
  }
  return acceptRideOffer({ rideRequestId:input.rideRequestId,offerId:input.offerId,actorPersonId:identity.personId });
}

export async function transitionRideAsCurrentUser(identity: SessionIdentity, input:{rideId:string;toStatus:string;reason?:string|null}) {
  const db = dbRequired();
  if (['driver_en_route','arrived','picked_up','completed'].includes(input.toStatus)) {
    const ride = await db.query(`select driver_person_id from rides where id=$1`, [input.rideId]);
    if (!ride.rowCount) throw new Error("Ride not found");
    if (ride.rows[0].driver_person_id !== identity.personId) throw new Error("Only the assigned driver can update the driving status");
  }
...
export async function refreshUserMatches(identity: SessionIdentity, rideRequestId:string) {
  const db = dbRequired();
  const request = await db.query(`select requester_person_id,passenger_person_id from ride_requests where id=$1`, [rideRequestId]);
  if (!request.rowCount) throw new Error("Ride request not found");
...
export async function viewRideLocation(identity: SessionIdentity, input:{locationId:string;rideId?:string|null}) {
