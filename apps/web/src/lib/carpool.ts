  ...
  await client.query(
    `with ranked as (
       select id,(row_number() over (
         order by case when stop_type='pickup' then 0 else 1 end, planned_at nulls last,created_at,id
       )*10)::integer as new_sequence
       from ride_stops where ride_id=$1 and status<>'skipped'
     )
  ...
  try {
    ...
    const rideResult = await client.query(
      `select r.*,primary_rr.direction as primary_direction,primary_rr.requested_pickup_at as primary_pickup_at,
              dos.allow_multi_passenger,dos.max_detour_minutes
       from rides r
       ...
       where r.id=$1 for update`, [input.rideId]);
...
export async function removeRequestFromRide(input: {rideId:string;rideRequestId:string;actorPersonId:string;reason?:string|null}) {
  const db = dbRequired();
  const client = await db.connect();
  try {
    await client.query('BEGIN');
    ...
    const assignment = await client.query(
      `update ride_request_assignments set status='cancelled',updated_at=now()
       where ride_id=$1 and ride_request_id=$2 and status='confirmed' returning seats_reserved`, [ride.id,request.id]);
...
export async function getRideManifest(rideId: string) {
  ...
  const ride = await db.query(
    `select r.*,p.display_name as driver_name,e.title as event_title,
     ...
     from rides r join people p on p.id=r.driver_person_id left join events e on e.id=r.event_id where r.id=$1`, [rideId]);
  ...
  const passengers = await db.query(
    `select rp.*,p.display_name,p.preferred_name,rr.public_ref as request_ref,rr.direction,
     ...
     from ride_passengers rp join people p on p.id=rp.person_id left join ride_requests rr on rr.id=rp.ride_request_id
     where rp.ride_id=$1 and rp.assignment_status='confirmed'
     order by coalesce(rr.requested_pickup_at,rr.created_at)`, [rideId]);
  const stops = await db.query(
    `select rs.*,pl.generalized_area,pl.generalized_latitude,pl.generalized_longitude,p.display_name
     from ride_stops rs left join private_locations pl on pl.id=rs.private_location_id left join people p on p.id=rs.person_id
     where rs.ride_id=$1 and rs.status<>'skipped' order by rs.sequence`, [rideId]);
...
export async function listPoolableRides(organizationId: string) {
  ...
  const result = await db.query(
    `select r.*,e.title as event_title,p.display_name as driver_name,rr.direction,
     ...
     from rides r join ride_requests rr on rr.id=r.ride_request_id join people p on p.id=r.driver_person_id
     left join events e on e.id=r.event_id
     where r.organization_id=$1 and r.status='confirmed' and r.pooling_enabled=true
     order by coalesce(r.scheduled_pickup_at,e.starts_at),r.created_at`, [organizationId]);
  return result.rows;
}
