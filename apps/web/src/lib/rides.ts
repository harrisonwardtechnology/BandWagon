...
async function assertActiveMembership(organizationId: string, personId: string) {
  const db = dbRequired();
  const result = await db.query(
    `select 1 from memberships where organization_id=$1 and person_id=$2 and status='active' limit 1`,
    [organizationId, personId]
  );
  if (!result.rowCount) throw new Error("Person is not an active member of this organization");
}

async function assertEventRideEnabled(organizationId: string, eventId: string | null | undefined) {
  if (!eventId) return;
  const db = dbRequired();
  const result = await db.query(
    `select id,title,starts_at,ends_at from events
     where id=$1 and organization_id=$2 and status='active' and ride_coordination_enabled=true`,
    [eventId, organizationId]
  );
  if (!result.rowCount) throw new Error("Event is not available for ride coordination");
  return result.rows[0];
}

async function guardianApprovalFor(passengerPersonId: string, requesterPersonId: string) {
  const db = dbRequired();
  const person = await db.query(`select person_type,student_approval_required from people where id=$1`, [passengerPersonId]);
  if (!person.rowCount) throw new Error("Passenger not found");
  if (person.rows[0].person_type !== 'minor' || !person.rows[0].student_approval_required) {
    return { status: 'not_required' as const, approvedBy: null as string | null };
  }
  const guardian = await db.query(
    ...
    [requesterPersonId, passengerPersonId]
  );
  if (guardian.rowCount) return { status: 'approved' as const, approvedBy: requesterPersonId };
  return { status: 'pending' as const, approvedBy: null as string | null };
}
...
export async function createRideRequest(input: {
  ...
  requestedPickupAt?: string | Date | null;
  requestedDropoffAt?: string | Date | null;
}) {
  const db = dbRequired();
  await assertActiveMembership(input.organizationId, input.requesterPersonId);
  await assertActiveMembership(input.organizationId, input.passengerPersonId);
  const event = await assertEventRideEnabled(input.organizationId, input.eventId);
  const approval = await guardianApprovalFor(input.passengerPersonId, input.requesterPersonId);
  const status = approval.status === 'pending' ? 'pending_approval' : 'open';

  const result = await db.query(
    `insert into ride_requests
      (organization_id,event_id,requester_person_id,passenger_person_id,direction,seats_needed,
       pickup_note,dropoff_note,requested_pickup_at,requested_dropoff_at,guardian_approval_status,
  ...
  await db.query(
    ...
    [result.rows[0].id, input.requesterPersonId, status, JSON.stringify({ eventTitle: event?.title || null })]
  );
  return result.rows[0];
}

export async function approveRideRequest(input: { rideRequestId: string; guardianPersonId: string; approve: boolean }) {
  const db = dbRequired();
  const current = await db.query(`select * from ride_requests where id=$1`, [input.rideRequestId]);
  ...
  if (!['pending_approval','open'].includes(request.status)) throw new Error("Ride request can no longer be approved");
  const nextStatus = input.approve ? 'open' : 'cancelled';
  const result = await db.query(
    `update ride_requests set guardian_approval_status=$1,approved_by_person_id=$2,approved_at=now(),status=$3,updated_at=now()
     where id=$4 returning *`,
    [input.approve ? 'approved' : 'denied', input.guardianPersonId, nextStatus, input.rideRequestId]
  );
  await db.query(
    `insert into ride_status_events (ride_request_id,actor_person_id,event_type,from_status,to_status)
     values ($1,$2,$3,$4,$5)`,
    [input.rideRequestId,input.guardianPersonId,input.approve ? 'guardian_approved' : 'guardian_denied',request.status,nextStatus]
  );
  return result.rows[0];
}
...
export async function createRideOffer(input: {
  ...
  seatsOffered?: number;
  note?: string | null;
  proposedPickupAt?: string | Date | null;
}) {
  const db = dbRequired();
  const requestResult = await db.query(`select * from ride_requests where id=$1`, [input.rideRequestId]);
  if (!requestResult.rowCount) throw new Error("Ride request not found");
  const request = requestResult.rows[0];
  if (request.status !== 'open' || request.guardian_approval_status === 'pending') throw new Error("Ride request is not open for offers");
  ...
  const offer = await db.query(
    `insert into ride_offers (ride_request_id,driver_person_id,seats_offered,note,proposed_pickup_at,status)
     values ($1,$2,$3,$4,$5,'offered')
     on conflict (ride_request_id,driver_person_id) do update set seats_offered=excluded.seats_offered,note=excluded.note,
       proposed_pickup_at=excluded.proposed_pickup_at,status='offered',updated_at=now()
     returning *`,
...
export async function acceptRideOffer(input: { rideRequestId: string; offerId: string; actorPersonId: string }) {
  const db = dbRequired();
  ...
  try {
    ...
    const authorized = input.actorPersonId === request.requester_person_id || (await client.query(
      `select 1 from guardian_relationships where guardian_person_id=$1 and minor_person_id=$2 and can_approve_rides=true`,
      [input.actorPersonId,request.passenger_person_id]
    )).rowCount;
    if (!authorized) throw new Error("Person is not authorized to accept an offer for this request");

    const offerResult = await client.query(
      `select * from ride_offers where id=$1 and ride_request_id=$2 and status='offered' for update`,
      [input.offerId,input.rideRequestId]
    );
    ...
    const rideResult = await client.query(
      `insert into rides (organization_id,event_id,ride_request_id,accepted_offer_id,driver_person_id,status,scheduled_pickup_at)
       values ($1,$2,$3,$4,$5,'confirmed',coalesce($6,$7)) returning *`,
      [request.organization_id,request.event_id,request.id,offer.id,offer.driver_person_id,offer.proposed_pickup_at,request.requested_pickup_at]
    );
    const ride = rideResult.rows[0];
    await client.query(`insert into ride_passengers (ride_id,person_id,ride_request_id) values ($1,$2,$3)`, [ride.id,request.passenger_person_id,request.id]);
    await client.query(`update ride_requests set status='matched',updated_at=now() where id=$1`, [request.id]);
    await client.query(`update ride_offers set status=case when id=$1 then 'accepted' else 'declined' end,updated_at=now() where ride_request_id=$2 and status='offered'`, [offer.id,request.id]);
    await client.query(
      `insert into ride_status_events (ride_id,ride_request_id,actor_person_id,event_type,from_status,to_status,metadata)
       values ($1,$2,$3,'ride_matched','open','confirmed',$4::jsonb)`,
...
export async function transitionRide(input: { rideId: string; actorPersonId: string; toStatus: string; reason?: string | null }) {
  const db = dbRequired();
  const current = await db.query(`select r.*,rr.requester_person_id,rr.passenger_person_id from rides r join ride_requests rr on rr.id=r.ride_request_id where r.id=$1`, [input.rideId]);
  if (!current.rowCount) throw new Error("Ride not found");
  ...
  const allowed = input.actorPersonId === ride.driver_person_id || input.actorPersonId === ride.requester_person_id || (await db.query(
    `select 1 from guardian_relationships where guardian_person_id=$1 and minor_person_id=$2 and can_approve_rides=true`,
    [input.actorPersonId,ride.passenger_person_id]
  )).rowCount;
  if (!allowed) throw new Error("Person is not authorized to update this ride");
  ...
  const result = await db.query(
    ...
    [input.toStatus,input.reason || null,input.rideId]
  );
  await db.query(
    `insert into ride_status_events (ride_id,ride_request_id,actor_person_id,event_type,from_status,to_status,metadata)
...
export async function listRideRequests(organizationId: string) {
  const db = dbRequired();
  const result = await db.query(
...
export async function listRides(organizationId: string) {
  const db = dbRequired();
  const result = await db.query(
