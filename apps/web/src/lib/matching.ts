...
async function driverHasConflict(organizationId: string, driverPersonId: string, targetAt: Date) {
  const db = dbRequired();
  ...
  const result = await db.query(
    `select 1
     ...
     where r.organization_id=$1 and r.driver_person_id=$2
       and r.status in ('confirmed','driver_en_route','arrived','picked_up')
       and coalesce(r.scheduled_pickup_at,e.starts_at) is not null
       and abs(extract(epoch from (coalesce(r.scheduled_pickup_at,e.starts_at)-$3::timestamptz))/60)<=$4
     limit 1`,
    [organizationId,driverPersonId,targetAt,windowMinutes]
  );
...
async function driverSuggestions(request: any) {
  ...
  const drivers = await db.query(
    `select dp.*,p.display_name,
     ...
     where dp.status='active' and dp.person_id<>$2 and dp.person_id<>$3`,
  ...
  for (const driver of drivers.rows) {
    ...
    if (await driverHasConflict(request.organization_id,driver.person_id,targetAt)) continue;
    const available = await isDriverAvailable({
      organizationId:request.organization_id,
      driverPersonId:driver.person_id,
      at:targetAt,
    ...
    suggestions.push({
      ...
      rationale:{
        ...
        willingByDefault:Boolean(driver.willing_by_default),
        zone:zoneLabel,
        pickupArea:request.pickup_area || null,
        capacity:Number(driver.default_capacity),
        seatsNeeded:Number(request.seats_needed),
        multiPassenger:Boolean(driver.allow_multi_passenger),
      },
    });
  }
  return suggestions;
...
async function existingRideSuggestions(request: any) {
  ...
  const rides = await db.query(
    `select r.*,primary_rr.direction as primary_direction,primary_rr.requested_pickup_at as primary_pickup_at,
            ...
            p.display_name as driver_name,dp.max_detour_minutes,dp.max_pickup_radius_km,dp.allow_multi_passenger
     from rides r
     ...
     where r.organization_id=$1 and r.status='confirmed' and r.pooling_enabled=true
       and coalesce(dp.allow_multi_passenger,true)=true
       and ((r.event_id is null and $2::uuid is null) or r.event_id=$2::uuid)
       and primary_rr.direction=$3
       and r.seats_reserved+$4<=r.capacity_snapshot`,
  ...
  for (const ride of rides.rows) {
    ...
    if (request.pickup_latitude != null && request.pickup_longitude != null && ride.primary_pickup_latitude != null && ride.primary_pickup_longitude != null) {
      distanceKm = haversineKm(
        Number(request.pickup_latitude),Number(request.pickup_longitude),
        Number(ride.primary_pickup_latitude),Number(ride.primary_pickup_longitude)
      );
    ...
    suggestions.push({
      candidateType:'existing_ride',
      driverPersonId:ride.driver_person_id,
      rideId:ride.id,
      score,
      distanceKm,
      timeGapMinutes:gap,
      remainingCapacity:remainingAfter,
      rationale:{
        driverName:ride.driver_name,
        existingCarpool:true,
        timeGapMinutes:gap,
        seatsAvailableBefore:remainingBefore,
        seatsAvailableAfter:remainingAfter,
        pickupArea:request.pickup_area || null,
      },
    });
  }
...
export async function generateMatchSuggestions(input: {
  ...
  try {
    ...
    await client.query(
      `update match_suggestions set status='expired',updated_at=now()
       where ride_request_id=$1 and status='active'`,
      [request.id]
    );
    const run = await client.query(
      `insert into matching_runs
        (organization_id,ride_request_id,requested_by_person_id,candidate_count,suggestion_count,parameters)
       values ($1,$2,$3,$4,$5,$6::jsonb) returning *`,
      [request.organization_id,request.id,input.requestedByPersonId || null,all.length,selected.length,JSON.stringify({
        limit,
        maxPoolTimeGapMinutes:Number(process.env.MAX_POOL_TIME_GAP_MINUTES || 45),
        driverConflictWindowMinutes:Number(process.env.DRIVER_CONFLICT_WINDOW_MINUTES || 120),
      })]
    );
    const stored = [];
    for (const suggestion of selected) {
      const row = await client.query(
        ...
        [run.rows[0].id,request.organization_id,request.id,suggestion.candidateType,suggestion.driverPersonId,suggestion.rideId || null,
         suggestion.score,suggestion.distanceKm ?? null,suggestion.timeGapMinutes ?? null,suggestion.remainingCapacity,JSON.stringify(suggestion.rationale)]
      );
...
export async function listMatchSuggestions(organizationId: string, rideRequestId?: string | null) {
  ...
  if (rideRequestId) {
    values.push(rideRequestId);
    filter += ` and ms.ride_request_id=$2`;
  }
  const result = await db.query(
    `select ms.*,p.display_name as driver_name,rr.public_ref as request_ref,e.title as event_title,r.public_ref as ride_ref
     ...
     join people p on p.id=ms.driver_person_id
     join ride_requests rr on rr.id=ms.ride_request_id
     left join events e on e.id=rr.event_id
     left join rides r on r.id=ms.ride_id
     where ${filter}
     order by ms.ride_request_id,ms.score desc,ms.created_at desc`,
    values
  );
  return result.rows;
}
...
export async function dismissMatchSuggestion(suggestionId: string) {
  const db = dbRequired();
  const result = await db.query(
    `update match_suggestions set status='dismissed',updated_at=now() where id=$1 and status='active' returning *`,
    [suggestionId]
  );
...
export async function notifyTopDriverMatches(input: { rideRequestId: string; limit?: number }) {
  ...
  const rows = await db.query(
    `select ms.*,rr.organization_id,rr.public_ref,e.title as event_title
     ...
     order by ms.score desc limit $2`,
    [input.rideRequestId,limit]
  );
  const results = [];
  for (const row of rows.rows) {
    const result = await routeNotification({
      notificationType:'new_ride_available',
      title:'Ride help needed',
      body:`A BandWagon ride${row.event_title ? ` for ${row.event_title}` : ''} may be a good fit for you.`,
      personId:row.driver_person_id,
      organizationId:row.organization_id,
      url:`/rides/requests/${row.public_ref}`,
    }).catch((error)=>({ error:error instanceof Error ? error.message : 'notification failed' }));
