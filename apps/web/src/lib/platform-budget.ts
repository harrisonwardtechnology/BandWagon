...
export async function setPlatformBudget(input:{budgetMonth?:string;budgetCents:number;alertRecipients:string[];enabled?:boolean}){
 const db=dbRequired(),month=monthStart(input.budgetMonth),budget=Math.max(1,Math.round(input.budgetCents)),recipients=[...new Set((input.alertRecipients||[]).map(v=>v.trim().toLowerCase()).filter(Boolean))];
 return (await db.query(`insert into platform_cost_budgets(budget_month,budget_cents,alert_recipients,enabled,updated_at) values($1,$2,$3,$4,now()) on conflict(budget_month) do update set budget_cents=excluded.budget_cents,alert_recipients=excluded.alert_recipients,enabled=excluded.enabled,updated_at=now() returning *`,[month,budget,recipients,input.enabled!==false])).rows[0];
}
...
export async function getPlatformCostSnapshot(budgetMonth?:string){
 ...
 const aiCents=Math.round(Number(ai.rows[0]?.value||0)/10000),routingCents=Math.round(Number(routing.rows[0]?.value||0)/10000),messagingCents=Math.round(Number(messaging.rows[0]?.value||0));
 ...
 const totalCents=aiCents+routingCents+messagingCents+rideCents+validationCents,b=budget.rows[0]||null,percent=b?Math.round(totalCents/Number(b.budget_cents)*1000)/10:0;
 return{month,budget:b,costs:{aiCents,routingCents,messagingCents,rideCents,validationCents,totalCents},percent};
}
