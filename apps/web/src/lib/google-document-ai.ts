...
async function accessToken(){
  ...
  const response=await fetch("https://oauth2.googleapis.com/token",{method:"POST",headers:{"content-type":"application/x-www-form-urlencoded"},body:new URLSearchParams({grant_type:"urn:ietf:params:oauth:grant-type:jwt-bearer",assertion:`${unsigned}.${signature}`}),cache:"no-store",signal:AbortSignal.timeout(aiRequestTimeoutMs(process.env.AI_REQUEST_TIMEOUT_MS))});
...
export async function analyzeDriverLicenseDocument(input:{documentId:string;organizationId?:string|null;personId:string;storageKey:string;contentType:string}){
  ...
  const {jobId}=await beginGovernedAiJob({organizationId:input.organizationId,personId:input.personId,documentId:input.documentId,purpose:'driver_license_review',feature:'document_review',providerPath:'google_document_ai',modelAlias:'specialized',promptVersion:'driver-license-v1',reservationMicrousd:costMicrousd});
  try{
    const processorName=env("GOOGLE_DOCUMENT_AI_PROCESSOR_NAME");const object=await getPrivateObjectBytes(input.storageKey);if(object.bytes.length>10*1024*1024)throw new Error("Document is too large for automated processing");const token=await accessToken();
    const endpoint=(process.env.GOOGLE_DOCUMENT_AI_ENDPOINT||"https://documentai.googleapis.com").replace(/\/$/,"");
    const response=await fetch(`${endpoint}/v1/${processorName}:process`,{method:"POST",headers:{authorization:`Bearer ${token}`,"content-type":"application/json"},body:JSON.stringify({rawDocument:{content:object.bytes.toString("base64"),mimeType:input.contentType}}),cache:"no-store",signal:AbortSignal.timeout(aiRequestTimeoutMs(process.env.AI_REQUEST_TIMEOUT_MS))});
    const body:any=await response.json().catch(()=>({}));if(!response.ok)throw new Error(`Document AI request failed (${response.status})`);
    ...
    await db.query(`update ai_jobs set status='completed',result_json=$1::jsonb,confidence=$2,human_review_required=true,estimated_cost_microusd=$3,reserved_cost_microusd=0,completed_at=now(),updated_at=now() where id=$4`,[JSON.stringify(result),confidence,costMicrousd,jobId]);
  ...
  }catch(error){const timedOut=error instanceof Error&&(error.name==='TimeoutError'||error.name==='AbortError');await recordAiFallback({jobId,organizationId:input.organizationId,personId:input.personId,purpose:'driver_license_review',reason:timedOut?'Document AI timed out':'Document AI provider failed',timedOut});throw new Error(timedOut?"Document processing timed out; continue with manual review":"Document processing is unavailable; continue with manual review");}
}
