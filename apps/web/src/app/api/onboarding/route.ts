...
export async function GET() {
  try {
    ...
    return NextResponse.json({ ok:true, identity, context:await getOnboardingContext(identity.personId) });
  } catch (error) {
    ...
    switch (body.action) {
      case "add_student":
        result = await addStudentToHousehold({
          managerPersonId:identity.personId,
      ...
      case "join_organization":
        result = await joinOrganizationWithCode({ personId:identity.personId,code:String(body.code || "") });
        break;
      ...
      case "configure_driver":
        result = await configureSelfAsDriver({
          ...
          willingByDefault:body.willingByDefault !== false,
        });
        break;
      default:
        return NextResponse.json({ error:"Unknown action" }, { status:400 });
    }
    return NextResponse.json({ ok:true,result,context:await getOnboardingContext(identity.personId) });
  } catch (error) {
