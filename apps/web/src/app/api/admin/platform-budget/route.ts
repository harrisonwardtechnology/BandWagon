...
export async function POST(request:Request){
 ...
 const budgetDollars=Number(body.budgetDollars||0);if(!Number.isFinite(budgetDollars)||budgetDollars<=0)return Response.json({error:"budgetDollars must be greater than zero"},{status:400});
 ...
 const budget=await setPlatformBudget({budgetMonth:body.month||undefined,budgetCents:Math.round(budgetDollars*100),alertRecipients:recipients,enabled:body.enabled!==false});
