...
import { getAdminNotificationHealth } from "@/lib/admin-notification-health";

export const runtime="nodejs";
export const dynamic="force-dynamic";

async function dashboardWithHealth(identity:any,organizationId:string){
  const [dashboard,notifications]=await Promise.all([
    getOperationsDashboard(identity,organizationId),
    getAdminNotificationHealth(identity,organizationId)
  ]);
  return {...dashboard,notifications,economics:{...dashboard.economics,messagingCostCents:notifications.totalCostCents,estimatedOperatingCostCents:dashboard.economics.estimatedOperatingCostCents+notifications.totalCostCents}};
}
...
export async function GET(request:Request){
  try{
    ...
    return NextResponse.json({ok:true,organizations,dashboard:await dashboardWithHealth(identity,organizationId)});
...
export async function POST(request:Request){
  try{
    ...
    return NextResponse.json({ok:true,result,dashboard:await dashboardWithHealth(identity,String(body.organizationId||""))});
