...
export default function CredentialsPage(){
  ...
  async function remove(documentId:string,label:string){if(!window.confirm(`Delete ${label}? This also removes it from organization eligibility reviews.`))return;setWorking(true);setMessage("Deleting credential from private storage…");const r=await fetch("/api/credentials",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"delete",documentId})});const x=await r.json().catch(()=>({}));setWorking(false);if(!r.ok){setMessage(x.error||"Unable to delete credential");return;}setMessage(x.result?.pendingStorageCleanup?"Credential hidden immediately; secure storage cleanup is queued.":"Credential deleted from private storage.");await load();}
