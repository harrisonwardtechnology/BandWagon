...
const publicUrl = "/api/review-package";
...
test("organization proposal is published and linked from public review surfaces", () => {
  ...
  const downloadRoute = fs.readFileSync("src/app/api/review-package/route.ts", "utf8");
  ...
  assert.match(downloadRoute, /application\/vnd\.openxmlformats-officedocument\.wordprocessingml\.document/);
});
